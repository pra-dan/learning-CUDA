...
__global__ void calculation(int *d_d, int *d_i, int numElements)
{
    ...
    if(d_v == d_d[i]) 
        *d_i = i; // This may get overwritten multiple times if duplicates of target value are present
...
__host__ void copyFromHostToDevice(int h_v, int *h_d, int *d_d, int numElements)
{
    ...
    // Note: d_v was declared as a constant in the header file
...
__host__ void copyFromDeviceToHost(int *d_i, int *h_i, int numElements)
{
    ...
    // Copy the device result int in device memory to the host result int in host memory.
    // size_t size = numElements * sizeof(int);
...
int main(int argc, char *argv[])
{
    ...
    // Just to test positive case
    h_v = h_d[2]; // Saving work of using args. Comment out to see negative case

    printf("Searching for value: %d \n", h_v);
    auto[d_d, d_i] = allocateDeviceMemory(numElements);
    copyFromHostToDevice(h_v, h_d, d_d, numElements);

    // Also do memcpy for d_i
    int notFound = numElements; // initialize with max index (instead of -1)
    // Update d_i value in device version as it will be updated in device 
    cudaMemcpy(d_i, &notFound, sizeof(int), cudaMemcpyHostToDevice);
