...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    float *h_a;
    cudaHostAlloc((float **)&h_a, size, cudaHostAllocMapped);

    float *h_b;
    cudaHostAlloc((float **)&h_b, size, cudaHostAllocMapped);
...
__host__ void copyFromHostToDevice(float *h_a, float *h_b, float *d_a, float *d_b, int numElements)
{
    ...
    // cudaError_t err = cudaHostGetDevicePointer((float **)&d_a, (int *)h_a, size); <- I think this was the error
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy constant int d_b from host to device (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
}
...
__host__ void deallocateMemory(float *d_a, float *d_b)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "[182] Failed to free device vector d_b (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
